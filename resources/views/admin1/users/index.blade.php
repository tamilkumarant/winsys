<?php 
	$add=App\Helper::checkAuthFunction(4,'add'); 
	$edit=App\Helper::checkAuthFunction(4,'edit'); 
	$delete=App\Helper::checkAuthFunction(4,'delete'); 
				...
				<div class="col-md-12">	
					...
					<div class="box">
						...
						<div class="col-md-2 top2">	
							<a href="{{URL::to('user/add')}}" ><button class="btn btn-primary mr5">Add New</button></a>
						</div>
						</div>
						@endif
					<div class="box-header">
						...
						<tbody>
							...
							@foreach($users as $key=>$val)
								<tr>
									...
									<td>
										@if($edit)
										<a href="{{URL::to('user/status/'.$val->id)}}" title="Change Status" class="status btn @if($val->is_active==0){{'btn-success'}}@else{{'btn-danger'}}@endif">	
									...
									<td>
										@if($edit)
										<a href="{{URL::to('user/edit/'.$val->id)}}" title="Edit"><i class="fa fa-edit editIcon"></i></a>
										@endif
										@if($delete)
						   ...
						   <div class="modal-content">
							<form action="" method="post" class="cancel-order-form">
								...
								<div class="modal-body">
									<p class="cancel_order_text">Are you sure want to delete this User? You cannot undo this action!</p>
								</div>
...
<script>
	...
	function deleteRow(id){
		window.location.href = "{{URL::to('user/delete')}}/"+id;
	}
</script>
